...
class DecisionMaker():
	...
	def list_containers_by_host(host_ip):
		...
		try:
			...
			logger.info("Moving host {} from platform_nodes to failed_nodes".format(host_ip))
			### 2019.09.05
	...
	def update_platform_status(self):
		...
		###etcd way
			...
			try:
				...
				logger_2.error("Can't establish connection with {} host!".format(host))
	...
	def check_for_releasing_node(self):
		...
		"""
		thresholds = literal_eval(self.etcd_manager.read_key("/platform/orchestrator/thresholds"))
		orchestrator_conf = self.etcd_manager. \
							get_etcd_orchestrator_config()['platform']['orchestrator']
